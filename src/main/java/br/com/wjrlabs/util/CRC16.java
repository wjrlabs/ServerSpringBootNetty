...
public class CRC16 {
	/*
	 * irreducible polynomial 1 + x^2 + x^15 + x^16
	 */

	// Tabela de referencia
	private final static int[] CRC_16_TABLE = { 0x0000, 0xC0C1, 0xC181, 0x0140, 0xC301, 0x03C0, 0x0280, 0xC241, 0xC601,
	...
	/**
	 * 
	 * @param bytes to calculte Cyclic Redundancy Check (CRC16)
	 * @return 2 bytes - CRC16 resulting array (big endian)
	 * 
	 *         input = 123456789 output = bb3d
	 */
	public static byte[] calculate(byte[] bytes) {

		int crc = 0x0000;
		for (byte b : bytes) {
			crc = (crc >>> 8) ^ CRC_16_TABLE[(crc ^ b) & 0xff];
		}
